...
// width is unknown (Options.Width <= 0). It is large enough that any
// realistic `width ge N` / `width le N` breakpoint treats an unknown width
// as "unbounded" (content stays visible) rather than hiding anything.
const widthUnbounded = 1 << 30
...
func metricValue(name string, snap schema.StatusSnapshot, cfg config.ToolConfig, opts Options) (float64, bool) {
	...
	switch name {
	case "width":
		// Terminal width in columns, for width-conditional visibility
		// (when="width ge 80"). When the width is unknown (opts.Width == 0,
		// e.g. Claude Code < v2.1.153 or a non-subagent render with no
		// COLUMNS), report an effectively unbounded width so `width ge N`
		// stays true and nothing is hidden — an unknown width must never
		// blank content.
		if opts.Width <= 0 {
			return widthUnbounded, true
		}
		return float64(opts.Width), true
	...
	case "extra-usage-cost-usd":
		if snap.Account.ExtraUsage == nil || snap.Account.ExtraUsage.UsedCreditsUSD == nil {
			return 0, false
		}
		return *snap.Account.ExtraUsage.UsedCreditsUSD, true
	case "extra-usage-limit-usd":
		if snap.Account.ExtraUsage == nil || snap.Account.ExtraUsage.MonthlyLimitUSD == nil {
			return 0, false
		}
		return *snap.Account.ExtraUsage.MonthlyLimitUSD, true
	case "extra-usage-percent":
		if snap.Account.ExtraUsage == nil || snap.Account.ExtraUsage.Utilization == nil {
			return 0, false
		}
		return *snap.Account.ExtraUsage.Utilization, true
	case "seven-day-opus-percent":
		if snap.Account.SevenDayOpus == nil {
			return 0, false
		}
		return snap.Account.SevenDayOpus.UsedPercentage, true
	case "seven-day-sonnet-percent":
		if snap.Account.SevenDaySonnet == nil {
			return 0, false
		}
	...
	case "seven-day-sonnet-reset-minutes":
		return resetMinutes(snap.Account.SevenDaySonnet, opts.Now)
	case "task-token-count":
		if snap.Subagent == nil {
			return 0, false
		}
		return float64(snap.Subagent.TokenCount), true
	case "task-context-window-tokens":
		if snap.Subagent == nil {
			return 0, false
		}
		return float64(snap.Subagent.ContextWindowSize), true
	case "task-context-percent":
		if snap.Subagent == nil || snap.Subagent.ContextWindowSize <= 0 {
			return 0, false
		}
		return float64(snap.Subagent.TokenCount) / float64(snap.Subagent.ContextWindowSize) * 100, true
	case "task-duration-seconds":
		if snap.Subagent == nil || snap.Subagent.StartedAt.IsZero() || opts.Now.IsZero() {
			return 0, false
		}
		return opts.Now.Sub(snap.Subagent.StartedAt).Seconds(), true
	default:
		return 0, false
	}
}
