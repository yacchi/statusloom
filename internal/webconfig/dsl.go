...
func (s *server) handlePreviewDSL(w http.ResponseWriter, r *http.Request) {
	...
	snap, ok := s.previewSnapshot(w, req.Sample, req.SessionID)
	if !ok {
		return
	}
...
// sample (defaulting to "full"). It writes the error response and returns
// ok=false on failure.
func (s *server) previewSnapshot(w http.ResponseWriter, sample, sessionID string) (schema.StatusSnapshot, bool) {
	if sessionID != "" {
...
func (s *server) handleDSLFields(w http.ResponseWriter, r *http.Request) {
	...
	for _, f := range fields {
		out = append(out, dslFieldEntry{
			...
			Formats:      f.Formats,
			Preview:      previewFor(f.Name, snap, now),
		})
	}
