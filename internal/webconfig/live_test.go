...
func TestLive_ShutdownRemovesTmpDirs(t *testing.T) {
	t.Setenv("STATUSLOOM_CACHE_DIR", t.TempDir())
	ts := startTestServer(t, time.Hour)

	body := createLiveSession(t, ts)
	if _, err := os.Stat(body.TmpDir); err != nil {
		t.Fatalf("tmpDir should exist after creation: %v", err)
	}
	...
	if _, err := os.Stat(body.TmpDir); !os.IsNotExist(err) {
		t.Errorf("tmpDir still exists after shutdown (stat err = %v)", err)
	}
}
