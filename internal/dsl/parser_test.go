...
func TestParseFieldMinWidthAndAlign(t *testing.T) {
	doc := parseClean(t, wrap(`<field name="task-tokens" min-width="6" align="right"/>`))
	f := onlyLine(t, doc).Children[0].(*FieldNode)
	if f.MinWidth == nil || *f.MinWidth != 6 {
		t.Fatalf("min-width = %v, want 6", f.MinWidth)
	}
	if f.Align != "right" {
		t.Fatalf("align = %q, want %q", f.Align, "right")
	}

	// Unspecified min-width/align default to nil/"" (left-align, no padding).
	doc2 := parseClean(t, wrap(`<field name="model"/>`))
	f2 := onlyLine(t, doc2).Children[0].(*FieldNode)
	if f2.MinWidth != nil {
		t.Fatalf("default min-width = %v, want nil", f2.MinWidth)
	}
	if f2.Align != "" {
		t.Fatalf("default align = %q, want \"\"", f2.Align)
	}
}

func TestParseFieldMinWidthInvalidValue(t *testing.T) {
	for _, val := range []string{"0", "-1", "abc", ""} {
		doc, diags := Parse(wrap(`<field name="model" min-width="` + val + `"/>`))
		if doc == nil {
			t.Fatalf("min-width=%q: Parse returned nil doc", val)
		}
		if !hasErrorContaining(diags, "min-width") {
			t.Errorf("min-width=%q: expected a min-width diagnostic, got %v", val, diags)
		}
		f := onlyLine(t, doc).Children[0].(*FieldNode)
		if f.MinWidth != nil {
			t.Errorf("min-width=%q: MinWidth = %v, want nil on invalid input", val, f.MinWidth)
		}
	}
}
