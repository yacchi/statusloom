...
* プレリリースにつき後方互換性は不要。旧`config.json`は完全に廃止済み（読み込み・自動移行とも行わない）。他ツールからの移行はコード側importerを持たず、monitorワークスペースのCLAUDE.md playbookでLLMに委譲する
...
shared git設定は別ファイルにせず、各ツールファイルのroot直下のoptionalな`<git/>`要素として置く（→「git」参照）。旧`<configDir>/config.json`（tools map + shared git設定）は廃止済みで、読み込みも自動移行も行わない。
...
`min-width="N"`（正整数）は、fieldの**表示値**（prefix/suffixの外側、整形済みの値そのもの）を最低N桁までスペースでパディングする。値がN桁を超える場合は**切り詰めない**（min-widthセマンティクス、maxではなくmin）。`align`（`left`（省略時）または`right`）はパディングをどちら側へ置くかを選ぶ：`left`は値の右側（従来通り左寄せ表示）、`right`は値の左側（右寄せ表示）へ空白を追加する。

複数行で右側の数値系fieldの桁を揃えたい場合に使う（各行は独立描画で行間の幅共有ができないため）。パディング幅は東アジア表示幅対応済みの表示幅関数で計算するので、全角文字を含む値でも正しく揃う。`optional`/`when`でfield自体が非表示になる場合はパディングも出ない（既存の可視性挙動は不変）。
...
#### extra-usage / per-model weekly rate-limit field

Claude Codeのpay-as-you-go overage（サブスクリプション上限超過後に課金される従量利用分。「extra usage」）と、Opus/Sonnetそれぞれのper-model weekly rate-limitを表示するfield。

* `extra-usage-cost` — 当該課金期間のmetered pay-as-you-go費用（USD）。`format="currency"`
* `extra-usage-limit` — 設定された月次extra-usage上限額（USD）。`format="currency"`
* `extra-usage-percent` — extra-usage月次上限に対する消費率。`format="percent"`
* `weekly-usage-opus` — Opusモデルの7日間rate-limit使用率。`format="percent"`
* `weekly-usage-sonnet` — Sonnetモデルの7日間rate-limit使用率。`format="percent"`
* `weekly-reset-opus` — Opusの7日間windowリセットまでのcountdown。`format="countdown"`
* `weekly-reset-sonnet` — Sonnetの7日間windowリセットまでのcountdown。`format="countdown"`
...
### 表示幅計算（East Asian Width対応）

flex幅計算・compact判定・padding・min-width揃えは、すべて単一の表示幅関数（`internal/render`の`visibleWidth`/`runeWidth`）を経由する。ANSIエスケープシーケンスは幅0としてスキップし、それ以外のruneは以下のルールで幅を数える（外部依存なし、`unicode`パッケージの範囲判定＋自前レンジで実装）:

* 全角（East Asian Wide/Fullwidth）と判定されるruneは幅2: CJK統合漢字・拡張A、CJK互換漢字、ひらがな/カタカナ、ハングル字母/音節、CJK記号と句読点、康煕部首/CJK部首補助、CJK互換（縦書き形含む）、全角形・全角記号
* それ以外（ASCII・一般記号・combining marksなど）は幅1

日本語のタスク説明文など全角文字を含む値でも、この幅関数を通せば`min-width`での桁揃えが正しく機能する。
...
12. （migrationは廃止。コード側importer・config.json自動移行は持たず、他ツールからの移行はmonitorワークスペースのCLAUDE.md playbookでLLMに委譲する）
