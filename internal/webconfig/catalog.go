...
// against the given sample snapshot. Segment.Text is the plain rendering and
// Segment.ANSI the ansi16-styled one, so one RenderDocument call yields both.
// tool only affects the synthetic doc's own tool="" attribute (rendering
// dispatches on field name alone, not on it); it is threaded through purely
// for semantic accuracy.
func previewFor(tool, fieldName string, snap schema.StatusSnapshot, now time.Time) widgetPreview {
	src := `<statusloom version="1" tool="` + tool + `" color-level="ansi16">` +
