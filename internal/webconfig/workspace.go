...
// monitorWorkspaceDir returns the stable path of the reused monitor
// workspace, <cacheDir>/monitor-workspace.
//
// Keeping this path stable (rather than a fresh temp dir per launch) is what
// makes Claude Code's folder-trust prompt a one-time thing: the user accepts
// trust once for this fixed directory (via the embedded terminal or the live
// monitor), and because the path no longer changes, subsequent launches are
// prompt-free. statusloom never writes trust flags itself - granting trust is
// the user's action.
func monitorWorkspaceDir() (string, error) {
	d, err := cache.Dir()
	if err != nil {
		return "", err
	}
	return filepath.Join(d, "monitor-workspace"), nil
}
...
// git config.
//
// If dir is already a git repo it returns immediately: the workspace is now a
// stable, reused directory, so we must not re-initialize or add a fresh commit
// on every launch.
...
The agent panel row has the same kind of preview, but built-in: no JSON file
is needed since ` + "`--preview`" + ` renders a bundled representative
tasks[] payload:
...
## Migrating from an existing status line tool

statusloom has no built-in importer. If the user is coming from another status
line tool (for example ccstatusline, whose config lives at
` + "`~/.config/ccstatusline/settings.json`" + `), translate it yourself:

1. Read the other tool's config file and, if you can, a sample of its rendered
   output, so you understand which fields it shows and in what order.
