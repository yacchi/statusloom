...
func renderContent(spec config.WidgetSpec, snap schema.StatusSnapshot, cfg config.ToolConfig, opts Options, compact bool) string {
	...
	case "extra-usage-cost":
		if snap.Account.ExtraUsage == nil || snap.Account.ExtraUsage.UsedCreditsUSD == nil {
			return ""
		}
		v := strconv.FormatFloat(*snap.Account.ExtraUsage.UsedCreditsUSD, 'f', 2, 64)
		if spec.RawValue {
			return v
		}
		return "$" + v

	case "extra-usage-limit":
		if snap.Account.ExtraUsage == nil || snap.Account.ExtraUsage.MonthlyLimitUSD == nil {
			return ""
		}
		v := strconv.FormatFloat(*snap.Account.ExtraUsage.MonthlyLimitUSD, 'f', 2, 64)
		if spec.RawValue {
			return v
		}
		return "$" + v
	...
	case "extra-usage-percent":
		...
		return formatPercent(*snap.Account.ExtraUsage.Utilization)

	case "weekly-usage-opus":
		return usageWidget(snap.Account.SevenDayOpus)

	case "weekly-usage-sonnet":
		return usageWidget(snap.Account.SevenDaySonnet)

	case "weekly-reset-opus":
		return resetWidget(snap.Account.SevenDayOpus, opts.Now)
	...
	case "task-description":
		...
		if snap.Subagent.Description != "" {
			return snap.Subagent.Description
		}
		return snap.Subagent.Label

	case "task-model":
		if snap.Subagent == nil {
			return ""
		}
		return snap.Subagent.ModelDisplay

	case "task-model-id":
		if snap.Subagent == nil {
			return ""
		}
		return snap.Subagent.ModelID

	case "task-status":
		if snap.Subagent == nil {
			return ""
		}
		return snap.Subagent.Status

	case "task-tokens":
		if snap.Subagent == nil {
			return ""
		}
		return renderInteger(snap.Subagent.TokenCount, spec.RawValue, compact)

	case "task-context-size":
		if snap.Subagent == nil {
			return ""
		}
		return renderInteger(snap.Subagent.ContextWindowSize, spec.RawValue, compact)
	...
	case "task-duration":
		...
		d := opts.Now.Sub(snap.Subagent.StartedAt)
		if spec.RawValue {
	...
	case "task-effort":
		if snap.Subagent == nil || snap.Subagent.Effort == nil {
			return ""
		}
		return *snap.Subagent.Effort
