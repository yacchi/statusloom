...
> **注記（現行実装）**: 以下のJSON `config.json` スキーマは初期設計の歴史的記録であり、現行の設定モデルはDSLドキュメント `<tool>.xml`（`markup.md` が正）に置き換わっている。`config.json` は読み込まれず（自動移行も含めmigration経路は全廃済み）、保存先は `~/.config/statusloom/<tool>.xml`（Windowsは `%AppData%\statusloom\<tool>.xml`）。以降のJSON例は参考情報。
...
**コードベースにimporterは持たない**（旧 `statusloom import ccstatusline` と `config.json`→`<tool>.xml` 自動移行は削除済み）。他ツール（例: ccstatusline の `~/.config/ccstatusline/settings.json`）からの移行は、monitorワークスペースに常駐するコーディングエージェント（LLM）に委譲する。ワークスペースの生成 `CLAUDE.md`（`internal/webconfig/workspace.go` の `workspaceClaudeMD`「Migrating from an existing status line tool」節）に手順を焼き込む:

1. 元ツールの設定ファイルとサンプルレンダーを読み、表示Widgetと順序を把握する
2. 同じレイアウトをstatusloom DSL（`markup.md`）で表現する（各項目を `<field>` にマップし、区切り・ラベル・色・条件表示を再現）
3. `statusloom draft push` で共有ドラフトに載せ、`statusloom claude < sample.json`（または `monitor --draft`）でプレビューする
4. 元表示と比較し一致するまで反復、ユーザーがWeb UIから保存する
...
- config schema（DSLドキュメント）
- validation
- built-in default document
- 他ツールからの移行はLLM playbookに委譲（コード側importer・自動移行は持たない。17章参照）
...
16. Implement `statusloom claude`
17. Implement `statusloom setup claude-code`
18. Bootstrap React configurator
19. Add Go preview API
20. Embed frontend assets
21. Add terminal width preview
22. Add release workflow
23. Add benchmark suite
24. Add built-in presets
